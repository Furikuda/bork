...
def test_format_parse_roundtrip_bork1(key):  # legacy
    repo_objs = RepoObj1(key)
    data = b"foobar" * 10
    id = repo_objs.id_hash(data)
    meta = {}  # bork1 does not support this kind of metadata
    cdata = repo_objs.format(id, meta, data)

    # bork1 does not support separate metadata and bork2 does not invoke parse_meta for bork1 repos
...
def test_bork1_bork2_transition(key):
    # bork transfer reads bork 1.x repo objects (without decompressing them),
    # writes bork 2 repo objects (giving already compressed data to avoid compression).
    meta = {}  # bork1 does not support this kind of metadata
    data = b"foobar" * 10
    len_data = len(data)
    repo_objs1 = RepoObj1(key)
    id = repo_objs1.id_hash(data)
    bork1_cdata = repo_objs1.format(id, meta, data)
    meta1, compr_data1 = repo_objs1.parse(id, bork1_cdata, decompress=False)  # bork transfer avoids (de)compression
    # in bork 1, we can only get this metadata after decrypting the whole chunk (and we do not have "size" here):
    ...
    # here, we just use len_data. for bork transfer, we also know the size from another metadata source.
    bork2_cdata = repo_objs2.format(
        id, dict(meta1), compr_data1[2:], compress=False, size=len_data, ctype=meta1["ctype"], clevel=meta1["clevel"]
    )
    meta2, data2 = repo_objs2.parse(id, bork2_cdata)
    assert data2 == data
    assert meta2["ctype"] == LZ4.ID
    assert meta2["clevel"] == 0xFF
    assert meta2["csize"] == meta1["csize"] - 2  # bork2 does not store the type/level bytes there
    assert meta2["size"] == len_data

    meta2 = repo_objs2.parse_meta(id, bork2_cdata)
    # now, in bork 2, we have nice and separately decrypted metadata (no need to decrypt the whole chunk):
    assert meta2["ctype"] == LZ4.ID
    assert meta2["clevel"] == 0xFF
    assert meta2["csize"] == meta1["csize"] - 2  # bork2 does not store the type/level bytes there
    assert meta2["size"] == len_data
