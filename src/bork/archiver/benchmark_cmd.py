...
class BenchmarkMixIn:
    def do_benchmark_crud(self, args):
        ...
        def measurement_run(repo, path):
            ...
            rc = self.do_create(
                self.parse_args(
                    [f"--repo={repo}", "create", compression, "--files-cache=disabled", "bork-benchmark-crud1", path]
                )
            )
            ...
            rc1 = self.do_create(
                self.parse_args([f"--repo={repo}", "create", compression, "bork-benchmark-crud2", path])
            )
            rc2 = self.do_delete(self.parse_args([f"--repo={repo}", "delete", "-a", "bork-benchmark-crud2"]))
            assert rc1 == rc2 == 0
            # measure a no-change update (archive1 is still present)
            t_start = time.monotonic()
            rc1 = self.do_create(
                self.parse_args([f"--repo={repo}", "create", compression, "bork-benchmark-crud3", path])
            )
            t_end = time.monotonic()
            dt_update = t_end - t_start
            rc2 = self.do_delete(self.parse_args([f"--repo={repo}", "delete", "-a", "bork-benchmark-crud3"]))
            assert rc1 == rc2 == 0
            # measure extraction (dry-run: without writing result to disk)
            t_start = time.monotonic()
            rc = self.do_extract(self.parse_args([f"--repo={repo}", "extract", "bork-benchmark-crud1", "--dry-run"]))
            t_end = time.monotonic()
            dt_extract = t_end - t_start
            assert rc == 0
            # measure archive deletion (of LAST present archive with the data)
            t_start = time.monotonic()
            rc = self.do_delete(self.parse_args([f"--repo={repo}", "delete", "-a", "bork-benchmark-crud1"]))
            t_end = time.monotonic()
        ...
        def test_files(path, count, size, random):
            try:
                path = os.path.join(path, "bork-test-data")
                os.makedirs(path)
    ...
    def build_parser_benchmarks(self, subparsers, common_parser, mid_common_parser):
        ...
        subparser = benchmark_parsers.add_parser(
            ...
            formatter_class=argparse.RawDescriptionHelpFormatter,
            help="benchmarks bork CRUD (create, extract, update, delete).",
        )
        ...
        subparser = benchmark_parsers.add_parser(
            ...
            formatter_class=argparse.RawDescriptionHelpFormatter,
            help="benchmarks bork CPU bound operations.",
        )
