...
class BackupOSError(Exception):
    ...
    Bork does different kinds of IO, and IO failures have different consequences.
...
class Archive:
    ...
    def __init__(
        ...
        self.name_in_manifest = name  # can differ from .name later (if bork check fixed duplicate archive names)
        self.comment = None
...
""".format(
    ...
    def save(self, name=None, comment=None, timestamp=None, stats=None, additional_metadata=None):
        ...
        # because bork info relies on them. so, either use the given stats (from args)
    ...
    def extract_item(
        ...
        if dry_run or stdout:
            ...
            if has_damaged_chunks:
                raise BackupError("File has damaged (all-zero) chunks. Try running bork check --repair.")
            return
    ...
    def delete(self, stats, progress=False, forced=False):
        ...
        if error:
            ...
            logger.warning("bork check --repair is required to free all space.")
...
class MetadataCollector:
    ...
    def stat_simple_attrs(self, st):
        ...
        # bork can work with archives only having mtime (very old bork archives do not have
...
class TarfileObjectProcessors:
    ...
    def process_hardlink(self, *, tarinfo, status, type):
        with self.create_helper(tarinfo, status, type) as (item, status):
            # create a not hardlinked bork item, reusing the chunks, see HardLinkManager.__doc__
...
class ArchiveChecker:
    ...
    def verify_data(self):
        ...
        if defect_chunks:
            if self.repair:
                # if we kill the defect chunk here, subsequent actions within this "bork check"
    ...
    def rebuild_manifest(self):
        ...
        # lost manifest on a older bork version than the most recent one that was ever used
        # within this repository (assuming that newer bork versions support more item keys).
    ...
    def rebuild_refcounts(self, first=0, last=0, sort_by="", match=None):
        ...
        def verify_file_chunks(archive_name, item):
            ...
            if "size" in item:
                ...
                if item_size != item_chunks_size:
                    # just warn, but keep the inconsistency, so that bork extract can warn about it.
                    logger.warning(
                        "{}: {}: size inconsistency detected: size {}, chunks size {}".format(
...
class ArchiveRecreater:
    ...
    def create_target(self, archive, target_name=None):
        ...
        if len(source_chunker_params) == 4 and isinstance(source_chunker_params[0], int):
            # this is a bork < 1.2 chunker_params tuple, no chunker algo specified, but we only had buzhash:
