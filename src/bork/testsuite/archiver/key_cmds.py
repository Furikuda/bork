...
class ArchiverTestCase(ArchiverTestCaseBase):
    ...
    def test_key_import_keyfile_with_bork_key_file(self):
        self.cmd(f"--repo={self.repository_location}", "rcreate", KF_ENCRYPTION)
        ...
        assert not os.path.isfile(key_file), '"bork key import" should respect BORG_KEY_FILE'
...
"""
    ...
    def test_init_defaults_to_argon2(self):
        """https://github.com/furikuda/bork/issues/747#issuecomment-1076160401"""
        self.cmd(f"--repo={self.repository_location}", "rcreate", RK_ENCRYPTION)
        with Repository(self.repository_path) as repository:
...
@unittest.skipUnless("binary" in BORG_EXES, "no bork.exe available")
class ArchiverTestCaseBinary(ArchiverTestCaseBinaryBase, ArchiverTestCase):
    """runs the same tests, but via the bork binary"""
