...
- unicode_errors = 'surrogateescape'
  ...
  It is needed because bork also needs to deal with data that does not cleanly encode/decode using utf-8.
  There's a lot of crap out there, e.g. in filenames and as a backup tool, we must keep them as good as possible.

Unpacking
---------
- raw = False (used by bork since bork 2.0)
  We already can use this with bork 2.0 due to the type conversion to the desired type in item.py update_internal
...
- raw = True (the old way, used by bork < 1.3)

- unicode_errors = 'surrogateescape' -> see description above (will be used when raw is False).

As of bork 2.0, we have fixed most of the msgpack str/bytes mess, #968.
Bork now still needs to **read** old repos, archives, keys, ... so we can not yet fix it completely.
But from now on, bork only **writes** new data according to the new msgpack 2.0 spec,
thus we can remove some legacy support in a later bork release (some places are marked with "legacy").
