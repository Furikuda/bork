    ...
    def positional_to_named(self, method, argv):
        ...
        try:
            ...
            if method == "open" and len(argv) == 4:
                # bork clients < 1.0.7 use open() with 4 args
            ...
            if r:
                ...
                if not data:
                    if self.repository is not None:
                        ...
                        os_write(
                            stderr_fd,
                            "Bork {}: Got connection close before repository was opened.\n".format(
                                __version__
                            ).encode(),
                        )
                    return
    ...
    def open(
        ...
        if self.restrict_to_repositories:
            for restrict_to_repository in self.restrict_to_repositories:
                ...
                raise PathNotAllowed(path)
        # "bork init" on "bork serve --append-only" (=self.append_only) does not create an append only repo,
        # while "bork init --append-only" (=append_only) does, regardless of the --append-only (self.append_only)
        # flag for serve.
...
def api(*, since, **kwargs_decorator):
    ...
    <since> specifies the version in which bork introduced this method.
...
class RemoteRepository:
    ...
    class RPCError(Exception):
        def __init__(self, unpacked):
            # for bork < 1.1: unpacked only has 'exception_class' as key
            # for bork 1.1+: unpacked has keys: 'exception_args', 'exception_full', 'exception_short', 'sysinfo'
...
given to open(), you can ignore it if it comes from a bork version < 1.0.7.
This TypeError is a cosmetic side effect of the compatibility code bork
clients >= 1.0.7 have to support older bork servers.
...
"""
    ...
    def bork_cmd(self, args, testing):
        """return a bork serve command line"""
        # give some args/options to 'bork serve' process as they were given to us
        opts = []
        if args is not None:
...
def handle_remote_line(line):
    ...
    if line.startswith("{"):
        # This format is used by Bork since 1.1.0b6 for new-protocol clients.
        ...
        if msg["type"] == "log_message":
            ...
            if logging.getLogger("bork").json and level >= target_logger.getEffectiveLevel():
            ...
            # through the effective level of the bork.output.progress logger
            # (also see ProgressIndicatorBase in bork.helpers).
            progress_logger = logging.getLogger("bork.output.progress")
            if progress_logger.getEffectiveLevel() == logging.INFO:
                # When progress output is enabled, we check whether the client is in
                # --log-json mode, as signalled by the "json" attribute on the "bork" logger.
                if logging.getLogger("bork").json:
        ...
        # This format is used by bork serve 0.xx, 1.0.x and 1.1.0b1..b5.
...
class RepositoryCache(RepositoryNoCache):
    ...
    *pack* receives the output of *transform* and should return bytes,
    ...
    def __init__(self, repository, pack=None, unpack=None, transform=None):
        ...
        self.cache = set()
        self.basedir = tempfile.mkdtemp(prefix="bork-cache-")
