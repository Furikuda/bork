...
class HardLinkManager:
    ...
    A) When creating a bork2 archive from the filesystem, we have to maintain a mapping like:
    ...
    B) When extracting a bork2 archive to the filesystem, we have to maintain a mapping like:
       hlid -> path
       If we encounter the same hlid again later, we hardlink to the path of the already extracted content of same hlid.

    C) When transferring from a bork1 archive, we need:
       path -> chunks, chunks_healthy  # for bork1_hl_targets
    ...
    D) When importing a tar file (simplified 1-pass way for now, not creating bork hardlink items):
       path -> chunks
    ...
    def remember(self, *, id, info):
        ...
        :param id: some id used to reference to the contentful item, could be:
                   a path (tar style, old bork style) [bytes]
                   a hlid (new bork style) [bytes]
...
# see https://github.com/furikuda/bork/issues/6933 and https://github.com/furikuda/bork/issues/6400
