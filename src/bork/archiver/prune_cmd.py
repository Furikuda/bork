...
class PruneMixIn:
    ...
    def do_prune(self, args, repository, manifest):
        ...
        with Cache(repository, manifest, lock_wait=self.lock_wait, iec=args.iec) as cache:
            ...
            list_logger = logging.getLogger("bork.output.list")
            ...
            if sig_int:
                ...
                checkpoint_func()
            if args.stats:
                log_multi(str(stats), logger=logging.getLogger("bork.output.stats"))
        return self.exit_code
    ...
    def build_parser_prune(self, subparsers, common_parser, mid_common_parser):
        ...
        Important: Repository disk space is **not** freed until you run ``bork compact``.
        ...
        negative number of archives to keep means that there is no limit. As of bork
        1.2.0, bork will retain the oldest archive if any of the secondly, minutely,
