...
# in order to speed up `bork check --repair`, any tag greater than MAX_TAG_ID
# is assumed to be corrupted. When increasing this value, in order to add more
# tags, keep in mind that old versions of Bork accessing a new repository
...
class Repository:
    ...
    def __init__(
        ...
        # v2 is the default repo version for bork 2.0
        # v1 repos must only be used in a read-only way, e.g. for
        # --other-repo=V1_REPO with bork init and bork transfer!
    ...
    def is_repository(path):
        """Check whether there is already a Bork repository at *path*."""
        try:
            ...
            with open(os.path.join(path, "README"), "rb") as fd:
                ...
                return b"Bork Backup repository" in readme_head or b"Bork repository" in readme_head
        except OSError:
    ...
    def check_can_create_repository(self, path):
        ...
        (1) It's just a weird thing to do, and usually not intended. A Bork using the "parent" repository
        ...
        while True:
            # Check all parent directories for Bork's repository README
            previous_path = path
    ...
    def open(self, path, exclusive, lock_wait=None, lock=True):
        ...
        if self.version not in self.acceptable_repo_versions:
            ...
            raise self.InvalidRepositoryConfig(
                path, "repository version %d is not supported by this bork version" % self.version
            )
    ...
    def scan(self, limit=None, state=None):
        ...
        - if we are called from a bork check command, self.index is a valid, fresh, in-sync repo index.
...
class LoggedIO:
    ...
    def _read(self, fd, header, segment, offset, acceptable_tags, read_data=True):
        ...
        if size > MAX_OBJECT_SIZE:
            # if you get this on an archive made with bork < 1.0.7 and millions of files and
