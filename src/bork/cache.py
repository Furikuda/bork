...
files_cache_logger = create_logger("bork.debug.files_cache")
...
class SecurityManager:
    ...
    could be legitimately deleted, which is annoying because Bork didn't
...
def recanonicalize_relative_location(cache_location, repository):
    # bork < 1.0.8rc1 had different canonicalization for the repo location (see #1655 and #1741).
...
class CacheConfig:
    ...
    def load(self):
        ...
        try:
            ...
            if self._config.get("cache", "manifest") != self.integrity.pop("manifest"):
                ...
                self.integrity = {}
                logger.warning("Cache integrity data not available: old Bork version modified the cache.")
    ...
    def _check_upgrade(self, config_path):
        try:
            ...
            self.close()
            raise Exception("%s does not look like a Bork cache." % config_path) from None
...
class LocalCache(CacheStatsMixin):
    ...
    def open(self):
        if not os.path.isdir(self.path):
            raise Exception("%s Does not look like a Bork cache" % self.path)
