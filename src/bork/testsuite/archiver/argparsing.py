...
def test_get_args():
    ...
    args = archiver.get_args(
        ["bork", "serve", "--umask=0027", "--restrict-to-path=/p1", "--restrict-to-path=/p2"], "bork serve --info"
    )
    ...
    args = archiver.get_args(
        ["bork", "serve", "--restrict-to-repository=/r1", "--restrict-to-repository=/r2"],
        "bork serve --info --umask=0027",
    )
    assert args.restrict_to_repositories == ["/r1", "/r2"]
    # trying to cheat - break out of path restriction
    args = archiver.get_args(
        ["bork", "serve", "--restrict-to-path=/p1", "--restrict-to-path=/p2"], "bork serve --restrict-to-path=/"
    )
    assert args.restrict_to_paths == ["/p1", "/p2"]
    # trying to cheat - break out of repository restriction
    args = archiver.get_args(
        ["bork", "serve", "--restrict-to-repository=/r1", "--restrict-to-repository=/r2"],
        "bork serve --restrict-to-repository=/",
    )
    assert args.restrict_to_repositories == ["/r1", "/r2"]
    # trying to cheat - break below repository restriction
    args = archiver.get_args(
        ["bork", "serve", "--restrict-to-repository=/r1", "--restrict-to-repository=/r2"],
        "bork serve --restrict-to-repository=/r1/below",
    )
    assert args.restrict_to_repositories == ["/r1", "/r2"]
    # trying to cheat - try to execute different subcommand
    args = archiver.get_args(
        ["bork", "serve", "--restrict-to-path=/p1", "--restrict-to-path=/p2"], f"bork --repo=/ rcreate {RK_ENCRYPTION}"
    )
    assert args.func == archiver.do_serve
    ...
    args = archiver.get_args(["bork", "serve"], "BORG_FOO=bar bork serve --info")
    assert args.func == archiver.do_serve
