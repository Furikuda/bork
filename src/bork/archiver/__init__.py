# bork cli interface / toplevel archiver code
...
try:
    ...
    # an unhandled exception in the try-block would cause the bork cli command to exit with rc 1 due to python's
    # default behavior, see issue #4424.
    # as bork defines rc 1 as WARNING, this would be a mismatch, because a crash should be an ERROR (rc 2).
    traceback.print_exc()
...
class Archiver(
    ...
    class CommonOptions:
        ...
        def resolve(self, args: argparse.Namespace):  # Namespace has "in" but otherwise is not like a dict.
            ...
            for suffix in self.suffix_precedence:
                ...
                # "bork --debug create --info" shall result in --info being effective.
    ...
    def build_parser(self):
        ...
        parser = argparse.ArgumentParser(prog=self.prog, description="Bork - Deduplicated Backups", add_help=False)
        ...
        if parser.prog == "borkfs":
            return self.build_parser_borkfs(parser)
    ...
    def get_args(self, argv, cmd):
        """usually, just returns argv, except if we deal with a ssh forced command for bork serve."""
        result = self.parse_args(argv[1:])
        if cmd is not None and result.func == self.do_serve:
            # bork serve case:
            # - "result" is how bork got invoked (e.g. via forced command from authorized_keys),
            ...
            if client_result.func == result.func:
                ...
                # everything else comes from the forced "bork serve" command (or the defaults).
    ...
    def prerun_checks(self, logger, is_serve):
        if not is_serve:
            # this is the bork *client*, we need to check the python:
            check_python()
    ...
    def run(self, args):
        ...
        if args.show_version:
            logging.getLogger("bork.output.show-version").info("borkbackup version %s" % __version__)
        ...
        if not is_supported_msgpack():
            ...
            logger.error("Do not contact borkbackup support about this.")
...
def main():  # pragma: no cover
    ...
    with signal_handler("SIGINT", raising_signal_handler(KeyboardInterrupt)), signal_handler(
        ...
        if args.show_rc:
            rc_logger = logging.getLogger("bork.output.show-rc")
