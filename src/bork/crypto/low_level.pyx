...
data) is done to support the legacy envelope layout as used in attic and early bork
...
cdef class AES256_CTR_BASE:
    # Layout: HEADER + MAC 32 + IV 8 + CT (same as attic / bork < 1.3 IF HEADER = TYPE_BYTE, no AAD)
...
cdef class _AEAD_BASE:
    ...
    def decrypt(self, envelope, aad=b''):
        ...
        approx_block_count = self.block_count(len(envelope))  # sloppy, but good enough for bork
