    ...
    def update_internal(self, d):
        ...
        for k, v in list(d.items()):
            ...
            if k in ('user', 'group') and d[k] is None:
                # bork 1 stored some "not known" values with a None value.
                # bork 2 policy for such cases is to just not have the key/value pair.
                continue
            ...
            if k == 'xattrs':
                ...
                for xk, xv in list(v.items()):
                    xk = want_bytes(xk)
                    # old bork used to store None instead of a b'' value
