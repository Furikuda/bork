...
``bork serve`` has special support for ssh forced commands (see ``authorized_keys``
...
::
    ...
    # Allow an SSH keypair to only run bork, and only have access to /path/to/repo.
...
SSH Configuration
~~~~~~~~~~~~~~~~~
``bork serve``'s pipes (``stdin``/``stdout``/``stderr``) are connected to the ``sshd`` process on the server side. In the event that the SSH connection between ``bork serve`` and the client is disconnected or stuck abnormally (for example, due to a network outage), it can take a long time for ``sshd`` to notice the client is disconnected. In the meantime, ``sshd`` continues running, and as a result so does the ``bork serve`` process holding the lock on the repository. This can cause subsequent ``bork`` operations on the remote repository to fail with the error: ``Failed to create/acquire the lock``.
...
Replacing ``backupserver`` with the hostname, FQDN or IP address of the bork server.

This will cause the client to send a keepalive to the server every 10 seconds. If 30 consecutive keepalives are sent without a response (a time of 300 seconds), the ssh client process will be terminated, causing the bork process to terminate gracefully.
...
This will cause the server to send a keep alive to the client every 10 seconds. If 30 consecutive keepalives are sent without a response (a time of 300 seconds), the server's sshd process will be terminated, causing the ``bork serve`` process to terminate gracefully and release the lock on the repository.

If you then run bork commands with ``--lock-wait 600``, this gives sufficient time for the bork serve processes to terminate after the SSH connection is torn down after the 300 second wait for the keepalives to fail.
