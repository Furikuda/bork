...
from .. import __version_tuple__ as bork_version_tuple
...
def _replace_placeholders(text, overrides={}):
    ...
    data = {
        ...
        "borkversion": bork_version,
        "borkmajor": "%d" % bork_version_tuple[:1],
        "borkminor": "%d.%d" % bork_version_tuple[:2],
        "borkpatch": "%d.%d.%d" % bork_version_tuple[:3],
        **overrides,
    }
...
class ArchiveFormatter(BaseFormatter):
    ...
    def format_item_json(self, item):
        return json.dumps(self.get_item_data(item), cls=BorkJsonEncoder) + "\n"
...
class BorkJsonEncoder(json.JSONEncoder):
    def default(self, o):
...
def basic_json_data(manifest, *, cache=None, extra=None):
    key = manifest.key
    data = extra or {}
    data.update({"repository": BorkJsonEncoder().default(manifest.repository), "encryption": {"mode": key.ARG_NAME}})
...
def json_dump(obj):
    """Dump using BorkJSONEncoder."""
    return json.dumps(obj, sort_keys=True, indent=4, cls=BorkJsonEncoder)
