...
def setup_logging(stream=None, conf_fname=None, env_var="BORG_LOGGING_CONF", level="info", is_serve=False, json=False):
    ...
    the bork client log message interceptor.
    """
    global configured
    err_msg = None
    if env_var:
    ...
    if conf_fname:
        try:
            ...
            bork_logger = logging.getLogger("bork")
            bork_logger.json = json
    ...
    bork_logger = logging.getLogger("bork")
    bork_logger.formatter = formatter
    bork_logger.json = json
...
class LazyLogger:
    ...
    def __logger(self):
        if self.__real_logger is None:
            ...
            if self.__name.startswith("bork.debug.") and self.__real_logger.level == logging.NOTSET:
...
class JsonFormatter(logging.Formatter):
    RECORD_ATTRIBUTES = (
        ...
        "name",
        "message",
        # msgid is an attribute we made up in Bork to expose a non-changing handle for log messages
        "msgid",
    )
