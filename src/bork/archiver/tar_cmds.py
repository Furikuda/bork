...
class TarMixIn:
    ...
    def do_export_tar(self, args, repository, manifest, archive):
        ...
        # tarfile, stealing valuable CPU time from Bork and thus reducing throughput.
    ...
    def _export_tar(self, args, archive, tarstream):
        ...
        def item_to_tarinfo(item, original_path):
            """
            Transform a Bork *item* into a tarfile.TarInfo object.
        ...
        def item_to_paxheaders(format, item):
            """
            Transform (parts of) a Bork *item* into a pax_headers dict.
            """
            # PAX format
            # ----------
    ...
    def build_parser_tar(self, subparsers, common_parser, mid_common_parser):
        ...
        When giving '-' as the output FILE, Bork will write a tar stream to standard output.

        By default (``--tar-filter=auto``) Bork will detect whether the FILE should be compressed
        ...
        A ``--sparse`` option (as found in bork extract) is not supported.
