    ...
    def decorator(method):
        ...
        def wrapper(self, args, **kwargs):
            ...
            if not location.valid:  # location always must be given
                raise Error("missing repository, please use --repo or BORK_REPO env var!")
            ...
            with repository:
                if repository.version not in (2,):
                    raise Error(
                        "This bork version only accepts version 2 repos for -r/--repo. "
                        "You can use 'bork transfer' to copy archives from old to new repos."
                    )
        ...
        def wrapper(self, args, **kwargs):
            ...
            with repository:
                if repository.version not in (1, 2):
                    raise Error("This bork version only accepts version 1 or 2 repos for --other-repo.")
...
# e.g. through "bork ... --help", define a substitution for the reference here.
...
rst_plain_text_references = {
    ...
    "bork_patterns": '"bork help patterns"',
    "bork_placeholders": '"bork help placeholders"',
    "key_files": "Internals -> Data structures and file formats -> Key files",
    "bork_key_export": "bork key export --help",
}
...
def define_archive_filters_group(subparser, *, sort_by=True, first_last=True, oldest_newest=True, older_newer=True):
    ...
    group.add_argument(
        ...
        dest="match_archives",
        action=Highlander,
        help='only consider archive names matching the pattern. see "bork help match-archives".',
    )
...
def define_common_options(add_common_option):
    ...
    add_common_option(
        ...
        "The logger path is bork.debug.<TOPIC> if TOPIC is not fully qualified.",
    )
    ...
    add_common_option(
        ...
        dest="lock_wait",
        type=int,
        default=int(os.environ.get("BORK_LOCK_WAIT", 1)),
        action=Highlander,
    ...
    add_common_option("--show-version", dest="show_version", action="store_true", help="show/log the bork version")
