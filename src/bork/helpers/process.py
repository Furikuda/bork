...
# e.g. if this is interrupting "bork create", it shall try to create a checkpoint.
...
def ignore_sigint():
    ...
    getting killed (e.g. because it is still needed to cleanly shut down bork).
...
def prepare_subprocess_env(system, env=None):
    ...
    :param system: True for preparing to invoke system-installed binaries,
                   False for stuff inside the pyinstaller environment (like bork, python).
...
def create_filter_process(cmd, stream, stream_close, inbound=True):
    ...
    try:
        ...
        # something went wrong with processing the stream by bork
        logger.debug("Exception, killing the filter...")
        if cmd:
            proc.kill()
        bork_succeeded = False
        raise
    else:
        bork_succeeded = True
    finally:
        ...
        if cmd:
            ...
            if filter_stream_close:
                filter_stream.close()
            if bork_succeeded and rc:
                # if bork did not succeed, we know that we killed the filter process
