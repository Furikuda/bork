...
class UnsupportedKeyFormatError(Error):
    """Your bork key is stored in an unsupported format. Try using a newer version of bork."""
...
class KeyBase:
    ...
    # Name used in command line / API (e.g. bork init --encryption=...)
    ARG_NAME = "UNDEFINED"
    ...
    #
    # The empty passphrase is also special because Bork tries it first when no passphrase
    # was supplied, and if an empty passphrase works, then Bork won't ask for one.
    ...
    def _tam_key(self, salt, context):
        return hkdf_hmac_sha512(
            ...
            salt=salt,
            info=b"bork-metadata-authentication-" + context,
            output_length=64,
        )
    ...
    def create(cls, repository, args, *, other_key=None):
        ...
        if other_key is not None:
            ...
            if other_key.copy_crypt_key:
                ...
                # bork transfer re-encrypts all data anyway, thus we can default to a new, random AE key
...
class AEADKeyBase(KeyBase):
    ...
    def _get_session_key(self, sessionid):
        ...
        key = hkdf_hmac_sha512(
            ikm=self.crypt_key,
            salt=sessionid,
            info=b"bork-session-key-" + self.CIPHERSUITE.__name__.encode(),
            output_length=32,
        )
        return key
