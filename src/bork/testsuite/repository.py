...
class RepositoryAuxiliaryCorruptionTestCase(RepositoryTestCaseBase):
    ...
    def test_unknown_integrity_version(self):
        ...
        with open(integrity_path, "r+b") as fd:
            msgpack.pack(
                {
                    # Bork only understands version 2
                    b"version": 4.7
                },
                fd,
            )
...
class RemoteRepositoryTestCase(RepositoryTestCase):
    ...
    def test_bork_cmd(self):
        assert self.repository.bork_cmd(None, testing=True) == [sys.executable, "-m", "bork", "serve"]
        args = self._get_mock_args()
        ...
        assert self.repository.bork_cmd(args, testing=False) == ["bork", "serve", "--info"]
        args.remote_path = "bork-0.28.2"
        assert self.repository.bork_cmd(args, testing=False) == ["bork-0.28.2", "serve", "--info"]
        args.debug_topics = ["something_client_side", "repository_compaction"]
        assert self.repository.bork_cmd(args, testing=False) == [
            "bork-0.28.2",
            "serve",
            "--info",
        ...
        assert self.repository.bork_cmd(args, testing=False) == ["bork", "serve", "--info", "--storage-quota=314159265"]
        args.rsh = "ssh -i foo"
        self.repository._args = args
...
class RemoteLoggerTestCase(BaseTestCase):
    def setUp(self):
        ...
        logging.getLogger("bork.repository").handlers[:] = []
        logging.getLogger("bork.repository.foo").handlers[:] = []
        # capture stderr
        sys.stderr.flush()
    ...
    def test_pre11_format_messages(self):
        ...
        handle_remote_line("$LOG INFO Remote: bork < 1.1 format message\n")
        self.assert_equal(self.stream.getvalue(), "Remote: bork < 1.1 format message\n")
    ...
    def test_post11_format_messages(self):
        ...
        handle_remote_line("$LOG INFO bork.repository Remote: bork >= 1.1 format message\n")
        self.assert_equal(self.stream.getvalue(), "Remote: bork >= 1.1 format message\n")
        self.assert_equal(self.stderr.getvalue(), "")

    def test_remote_messages_screened(self):
        # default bork config for root logger
        self.handler.setLevel(logging.WARNING)
        logging.getLogger().setLevel(logging.WARNING)

        handle_remote_line("$LOG INFO bork.repository Remote: new format info message\n")
        self.assert_equal(self.stream.getvalue(), "")
        self.assert_equal(self.stderr.getvalue(), "")

    def test_info_to_correct_local_child(self):
        logging.getLogger("bork.repository").setLevel(logging.INFO)
        logging.getLogger("bork.repository.foo").setLevel(logging.INFO)
        # default bork config for root logger
        self.handler.setLevel(logging.WARNING)
        logging.getLogger().setLevel(logging.WARNING)
        ...
        logging.getLogger("bork.repository").handlers[:] = [child_handler]
        foo_stream = io.StringIO()
        foo_handler = logging.StreamHandler(foo_stream)
        foo_handler.setLevel(logging.INFO)
        logging.getLogger("bork.repository.foo").handlers[:] = [foo_handler]

        handle_remote_line("$LOG INFO bork.repository Remote: new format child message\n")
