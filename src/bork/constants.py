...
# in bork < 1.3, this has been defined like this:
# 20 MiB minus 41 bytes for a PUT header (because the "size" field in the Repository includes
# the header, and the total size was set to precisely 20 MiB for bork < 1.3).
MAX_DATA_SIZE = 20971479

# MAX_OBJECT_SIZE = MAX_DATA_SIZE + len(PUT2 header)
# note: for bork >= 1.3, this makes the MAX_OBJECT_SIZE grow slightly over the precise 20MiB used by
# bork < 1.3, but this is not expected to cause any issues.
MAX_OBJECT_SIZE = MAX_DATA_SIZE + 41 + 8  # see assertion at end of repository module

# How many segment files bork puts into a single directory by default.
...
# repo.list() / .scan() result count limit the bork client uses
...
FILES_CACHE_MODE_UI_DEFAULT = "ctime,size,inode"  # default for "bork create" command (CLI UI)
FILES_CACHE_MODE_DISABLED = "d"  # most bork commands do not use the files cache at all (disable)

# return codes returned by bork command
# when bork is killed by signal N, rc = 128 + N
...
class KeyType:
    ...
    KEYFILE = 0x00
    # repos with PASSPHRASE mode could not be created any more since bork 1.0, see #97.
    # in bork 2. all of its code and also the "bork key migrate-to-repokey" command was removed.
    # if you still need to, you can use "bork key migrate-to-repokey" with bork 1.0, 1.1 and 1.2.
    # Nowadays, we just dispatch this to RepoKey and assume the passphrase was migrated to a repokey.
    PASSPHRASE = 0x01  # legacy, bork < 1.0
    PLAINTEXT = 0x02
    REPO = 0x03
    BLAKE2KEYFILE = 0x04
    BLAKE2REPO = 0x05
...
REPOSITORY_README = """This is a Bork Backup repository.
See https://borkbackup.readthedocs.io/
"""

CACHE_README = """This is a Bork Backup cache.
See https://borkbackup.readthedocs.io/
"""
