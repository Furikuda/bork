...
If Bork is running in client/server mode, the client uses SSH as a transport to
talk to the remote agent, which is another Bork process (Bork is installed on
the server, too) started automatically by the client. The Bork server is doing
storage-related low-level repo operations (get, put, commit, check, compact),
while the Bork client does the high-level stuff: deduplication, encryption,
compression, dealing with archives, backups, restores, etc., which reduces the
amount of data that goes over the network.

When Bork is writing to a repo on a locally mounted remote file system, e.g.
SSHFS, the Bork client only can do file system operations and has no agent
...
``~/.cache/bork/``.  If Bork detects that a repository has been
...
Bork will keep an exclusive lock on the repository while creating
...
own repository by creating a new one with :ref:`bork_rcreate`.
...
- ``bork rcreate repo1 --encryption=X``
- ``bork rcreate repo2 --encryption=X --other-repo=repo1``
- maybe do a snapshot to have stable and same input data for both bork create.
- client machine ---bork create---> repo1
- client machine ---bork create---> repo2
...
The 2 independent bork create invocations mean that there is no error propagation
from repo1 to repo2 when done like that.

An alternative way would be to use ``bork transfer`` to copy backup archives
...
Warning: using bork with multiple repositories with identical repository ID (like when
...
::
  ...
  bork config id   # shows the REPO_ID
  rm ~/.config/bork/security/REPO_ID/manifest-timestamp
  bork rdelete --cache-only

This is an unsafe and unsupported way to use bork, you have been warned.
...
just invoke ``bork create`` as you always do. If the repository is still locked,
you may need to run ``bork break-lock`` before the next backup. You may use the
...
Bork always does full single-pass backups, so it will start again
...
You could do that (via bork config REPO append_only 0/1), but using different
...
::
    ...
    bork export-tar archive-name - | tar --compare -f - -C /path/to/compare/to


Can Bork add redundancy to the backup data to deal with hardware malfunction?
-----------------------------------------------------------------------------
...
Can Bork verify data integrity of a backup archive?
---------------------------------------------------
...
Can I use Bork on SMR hard drives?
----------------------------------
...
Bork ships with default settings suitable for SMR drives,
...
A single error does not necessarily indicate bad hardware or a Bork
...
(ii) Bugs in software (Bork, operating system, libraries):
...
Checking hard drives
  ...
  applications (e.g. Bork), while these errors can still corrupt data.
...
Checking memory
  Intermittent issues, such as ``bork check`` finding errors
...
:ref:`bork_check` provides diagnostics and ``--repair`` options for repositories with
...
it wouldn't be worth it and it would contradict Bork's design, so if you don't want this to
happen, simply don't use Bork.

Why is the time elapsed in the archive stats different from wall clock time?
----------------------------------------------------------------------------

Bork needs to write the time elapsed into the archive metadata before finalizing
the archive and committing the repo & cache.
This means when Bork is run with e.g. the ``time`` command, the duration shown
...
different prefixes. For example, you could have a script that does::
    ...
    bork create --exclude var/log main-$(date +%Y-%m-%d) /
    bork create logs-$(date +%Y-%m-%d) /var/log
...
A file is only removed from a BorkBackup repository if all archives that contain
...
1. Use :ref:`bork_delete` to remove all archives that contain the files. This
...
:ref:`bork_recreate` command to rewrite all archives with a different
``--exclude`` pattern. See the examples in the manpage for more information.

Finally, run :ref:`bork_compact` with the ``--threshold 0`` option to delete the
...
How important is the $HOME/.config/bork directory?
--------------------------------------------------
...
Using ``BORK_PASSCOMMAND`` with a file of proper permissions
  ...
  example, first create a key::
    ...
    (umask 0077; head -c 32 /dev/urandom | base64 -w 0 > ~/.bork-passphrase)
...
Using ``BORK_PASSCOMMAND`` with GNOME Keyring
  ...
  Then add a secret to the login keyring::
    ...
    head -c 32 /dev/urandom | base64 -w 0 | secret-tool store bork-repository repo-name --label="Bork Passphrase"
...
How do I report a security issue with Bork?
-------------------------------------------
...
You will also get this error if you try to access a repository with a key that uses the argon2 key algorithm using an old version of bork.
...
Why does Bork extract hang after some time?
-------------------------------------------

When I do a ``bork extract``, after a while all activity stops, no cpu usage,
no downloads.
...
If you have issues with lost connections during long-running bork commands, you
could try to work around:

- Make partial extracts like ``bork extract PATTERN`` to do multiple
  smaller extraction runs that complete before your connection has issues.
- Try using ``bork mount MOUNTPOINT`` and ``rsync -avH`` from
  ...
  to do any more. Due to the way bork mount works, this might be less efficient
  than bork extract for bigger volumes of data.
...
Why am I seeing idle bork serve processes on the repo server?
-------------------------------------------------------------
...
::
    ...
    # /etc/ssh/sshd_config on bork repo server - kill connection to client
    # after ClientAliveCountMax * ClientAliveInterval seconds with no response
    ClientAliveInterval 20
    ClientAliveCountMax 3

If you have multiple bork create ... ; bork create ... commands in a already
...
The bork cache eats way too much disk space, what can I do?
-----------------------------------------------------------

This may especially happen if bork needs to rebuild the local "chunks" index -
either because it was removed, or because it was not coherent with the
repository state any more (e.g. because another bork instance changed the
repository).

To optimize this rebuild process, bork caches per-archive information in the
...
that kept them with a file, so bork won't be able to store anything "in" there
in future.

This has some pros and cons, though:

- much less disk space needs for ~/.cache/bork.
...
Compared to simply copying files (e.g. with ``rsync``), Bork has more work to do.
This can make creation of the first archive slower, but saves time
and disk space on subsequent runs. Here what Bork does when you run ``bork create``:

- Bork chunks the file (using the relatively expensive buzhash algorithm)
...
- Stores the chunk into a key/value store (the key is the chunk id, the value
  ...
  checksum, used by bork check --repository) and also updates the repo index
  (another hashtable).
...
The observed performance is that Bork can process about
...
If you feel your Bork backup is too slow somehow, here is what you can do:

- Make sure Bork has enough RAM (depends on how big your repo is / how many
  files you have)
...
``--list --filter=AME --stats`` to your ``bork create`` call to produce more log output,
...
- ``A`` status ("added") in the file list:
  If you see that often, you have a lot of new files (files that Bork did not find
...
- ``M`` status ("modified") in the file list:
  If you see that often, Bork thinks that a lot of your files might be modified
  (Bork found them in the files cache, but the metadata read from the filesystem did
  not match the metadata stored in the files cache).
  In such a case, Bork will need to process the files' contents completely, which is
  much slower than processing unmodified files (Bork does not read their contents!).
...
When bork runs inside a virtual machine, there are some more things to look at:
...
The files cache is used to determine whether Bork already
...
the snapshot would not be contained in the next backup as Bork would
...
filesystem snapshot whose name is not stable), bork will assume that files are
...
Bork maintains a files cache where it remembers the timestamp, size and
inode of files. When Bork does a new backup and starts processing a
...
Is there a way to limit bandwidth with Bork?
--------------------------------------------

To limit upload (i.e. :ref:`bork_create`) bandwidth, use the
``--remote-ratelimit`` option.

There is no built-in way to limit *download*
(i.e. :ref:`bork_extract`) bandwidth, but limiting download bandwidth
...
::
    ...
    cd /mnt/rootfs
    bork create rootfs_backup .


I am having troubles with some network/FUSE/special filesystem, why?
--------------------------------------------------------------------

Bork is doing nothing special in the filesystem, it only uses very
...
the filesystem code you used (not with Bork). For this case, it is
...
issue in the Bork issue tracker about that.


Why does running 'bork check --repair' warn about data loss?
------------------------------------------------------------
...
In other words, the warning is there to emphasize that Bork:
...
running `bork check --repair` will fix most problems.


Why isn't there more progress / ETA information displayed?
----------------------------------------------------------

Some bork runs take quite a bit, so it would be nice to see a progress display,
...
But sometimes, the total amount is unknown (e.g. for ``bork create`` we just do
...
could not compute the ``bork create`` ETA as we do not know the amount of changed
chunks, how the bandwidth of source and destination or system performance might
fluctuate.

You see, trying to display ETA would be futile. The bork developers prefer to
...
checking for free disk space::
    ...
    bork config -- additional_free_space -2T
...
::
    ...
    bork config /path/to/repo storage_quota 0

If you are bound to the quota, you have to free repository space. The first to
try is running :ref:`bork_compact` to free unused backup space (see also
:ref:`separate_compaction`):

::

    bork compact /path/to/repo

If your repository is already compacted, run :ref:`bork_prune` or
:ref:`bork_delete` to delete archives that you do not need anymore, and then run
``bork compact`` again.

My backup disk is full, what can I do?
--------------------------------------

Bork cannot work if you really have zero free space on the backup disk, so the
first thing you must do is deleting some files to regain free disk space. See
:ref:`about_free_space` for further details.

Some Bork commands that do not change the repository might work under disk-full
...
In the bork git repository, there is scripts/glibc_check.py that can determine
